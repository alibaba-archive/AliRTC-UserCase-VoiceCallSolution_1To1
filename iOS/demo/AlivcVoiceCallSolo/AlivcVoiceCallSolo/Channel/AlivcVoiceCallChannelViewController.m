...
@interface AlivcVoiceCallChannelViewController ()<UITextFieldDelegate,AliRtcEngineDelegate>
...
/**
 @brief 加入房间button
 */
@property(nonatomic, strong) AlivcIndicatorButton  *btnJoin;
/**
 @brief 房间输入框的状态
 */
...
@implementation AlivcVoiceCallChannelViewController
...
- (void)onBtnJoin:(AlivcIndicatorButton *)sender {
    [sender startLoading];
...
/**
 @brief 加入频道
 */
- (void)joinChannel:(NSString *)channelName sender:(AlivcIndicatorButton *)sender {
    //AliRtcAuthInfo:各项参数均需要客户App Server(客户的server端) 通过OpenAPI来获取，然后App Server下发至客户端，客户端将各 项参数赋值后，即可joinChannel
    [AlivcVideoCallUserAuthrization getPassportFromAppServer:channelName  block:^(AliRtcAuthInfo *AuthInfo, NSError *error) {
        ...
        [AlivcVideoCallUserAuthrization userListInChannel:channelName block:^(NSArray *users, NSError *error) {
            if(error) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self showMessage:@"网络请求失败，请检查您的网络设置" inView:self.view];
                    [sender stopLoading];
            ...
            if (users.count <= 1) {
                ...
                [self.engine joinChannel:AuthInfo name:userName onResult:^(NSInteger errCode) {
                    ...
                    if (errCode == 0) {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            AlivcVoiceCallChattingViewController *chatVC = [[AlivcVoiceCallChattingViewController alloc] initWithChannelName:channelName appId:AuthInfo.appid userId:AuthInfo.user_id];
...
- (void)addSubviews {
    ...
    CGFloat logoW = 72;
    CGFloat logoH = 72;
    CGFloat logoX = (screenWidth - logoW)/2.0;
    CGFloat logoY = rect.size.height > 568 ?  150 : 80;
    ...
    logoView.image = [NSBundle pngImageWithName:@"logo"];
    
    //slogan
    UILabel *slogan = [[UILabel alloc] init];
    slogan.text = @"阿里云音视频 RTC";
    slogan.font = [UIFont fontWithName:@"PingFangSC-Medium" size:20];
    slogan.textColor = [UIColor colorWithRed:38/255.0 green:38/255.0 blue:38/255.0 alpha:1/1.0];
    [slogan sizeToFit];
    slogan.center =  CGPointMake(screenWidth * 0.5, CGRectGetMaxY(logoView.frame) + 30);
    [self.view addSubview:slogan];
    
    //subtitle
    UILabel *subtitle = [[UILabel alloc] init];
    subtitle.text = @"阿里云音视频通信RTC提供基础通信服务";
    subtitle.font = [UIFont fontWithName:@"PingFangSC-Regular" size:13];
    subtitle.textColor = [UIColor colorWithRed:38/255.0 green:38/255.0 blue:38/255.0 alpha:1/1.0];
    [subtitle sizeToFit];
    subtitle.center = CGPointMake(slogan.center.x, slogan.center.y + 30);
    [self.view addSubview:subtitle];
    
    //roomTitle
    CGFloat roomTitleX = 18;
    CGFloat roomTitleY = CGRectGetMaxY(subtitle.frame) +30;
    ...
    AlivcIndicatorButton *btnJoin = [AlivcIndicatorButton buttonWithType:UIButtonTypeCustom];
