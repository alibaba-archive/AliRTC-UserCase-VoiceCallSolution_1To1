...
@interface AlivcVoiceCallChattingViewController ()<AliRtcEngineDelegate,UIViewControllerTransitioningDelegate,AlivcModalViewControllerDelegate>
...
@brief 显示的Transition
*/
@property (strong,nonatomic) AlivcPresentTransition *presentTransition;
/**
@brief 关闭的Transition
*/
@property (strong,nonatomic) AlivcDismissTransition *dismissTransition;
/**
@brief 回退手势的Transition
*/
@property (strong,nonatomic) AlivcPanInteractiveTransition *panInteractiveTransition;
...
@implementation AlivcVoiceCallChattingViewController
...
*/
- (void)roomInfo {
    [AlivcVideoCallUserAuthrization channelStartTimeWithAppId:self.appId channel:self.channelName block:^(NSDate *startTime, NSError *error) {
        if (error) {
...
- (void)refreshRoomState:(void (^)(NSArray *users))block{
    [AlivcVideoCallUserAuthrization userListInChannel:self.channelName block:^(NSArray *users, NSError *error) {
        if (users.count == 2) {
...
- (void)settting {
    ...
    AlivcVoiceCallSettingViewController *vc = [[AlivcVoiceCallSettingViewController alloc] init];
...
- (AlivcPresentTransition *)presentTransition {
    if (!_presentTransition) {
        _presentTransition = [[AlivcPresentTransition alloc] init];
    }
    return _presentTransition;;
}

- (AlivcDismissTransition *)dismissTransition {
    if (!_dismissTransition) {
        _dismissTransition = [[AlivcDismissTransition alloc] init];
    }
    return _dismissTransition;
}

- (AlivcPanInteractiveTransition *)panInteractiveTransition {
    if (!_panInteractiveTransition) {
        _panInteractiveTransition = [[AlivcPanInteractiveTransition alloc] init];
    }
